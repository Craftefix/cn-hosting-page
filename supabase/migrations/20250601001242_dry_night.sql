...
-- Add-ons table
IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'subscription_addons') THEN
...
IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'billing_history') THEN
...
BEGIN
    ...
    -- subscription_addons policies
    DROP POLICY IF EXISTS "Users can view own addons" ON public.subscription_addons;
    DROP POLICY IF EXISTS "Service role can manage addons" ON public.subscription_addons;
